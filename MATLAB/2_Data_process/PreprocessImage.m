function PreprocessImage(dirr, camfoldername, image_format, save_dir, totalImgs)
if ~exist('save_dir', 'var')
    save_dir = dirr;
end

if ~exist('totalImgs', 'var')
    a = dir([dirr  'Cam1/*.tiff']);
    totalImgs = numel(a); 
end
...
middle_string = 'frame';
for i = 1 : ncams
    a = dir([dirr camfoldername num2str(i) '/*' image_format]);
    total_imgs(i) = numel(a); 
    filenamelist = {a.name};
    filenamelist = natsortfiles(filenamelist);
    for j = 1 : total_imgs(i)
        if exist([dirr camfoldername num2str(i) '/' cam_string num2str(i), middle_string num2str(j, '%05.0f') image_format], 'file')
            continue;
        end
        movefile([dirr camfoldername num2str(i) '/' filenamelist{j}], [dirr camfoldername num2str(i) '/' cam_string num2str(i), middle_string num2str(j, '%05.0f') image_format]);
    end
end
...
totalImgs_for_background = min(total_imgs);
if totalImgs_for_background > 1000
    totalImgs_for_background = 1000;
end
% ncams = 4;
img = imread([dirr camfoldername '1/' cam_string num2str(1) middle_string num2str(1,'%05.0f') image_format]); 
...
for cam = 1 : ncams
    num = 0;
    camdir = [dirr camfoldername num2str(cam) '/'];
    for I = 1:totalImgs_for_background
        if (mod(I,20) == 0)
            imgdir = [camdir cam_string num2str(cam) middle_string num2str(I, '%05.0f') image_format];
            img = imread(imgdir);
...
for cam = 1:ncams
    ...
    h_end = 3;
    t = 0;
    imgdir = [camdir cam_string num2str(cam) middle_string num2str(5,'%05.0f') image_format]; % TODO:
    img = imread(imgdir);
...
%         img3 = img - bak(:, :, cam);
        ...
        t = t + 1;
        if t > 15 || abs(threshold(cam) - 1) < 1e-4 % exit when it exceeds a number of loop times 
            break;
        end
    end
    parfor I = 1:1:total_imgs(cam)
        imgdir = [camdir cam_string num2str(cam) middle_string num2str(I, '%05.0f') image_format];
