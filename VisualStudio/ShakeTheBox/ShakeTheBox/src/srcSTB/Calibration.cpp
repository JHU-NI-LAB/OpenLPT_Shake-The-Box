...
//	for (long int p = 0; p < cleanlist[0].size(); p++) {
	for (long int p = 0; p < cleanlist_num; p++) {
...
//		cout << "Cannot open file\n";
	...
	double* points_array = new double[rows * cols];
	...
	for (int id = 0; id < num_camera; id++) {
		...
		for (int i = 0; i < num_points; i++) {
			Position pos(points_array[(2 * id + 3) * num_camera + i], points_array[(2 * id + 4) * num_camera + i], 0);
			pos2D_pixel.Add(pos);
			pos2D.Add(cams[id].UnDistort(pos));
		}
		corrFrames.push_back(pos2D);
