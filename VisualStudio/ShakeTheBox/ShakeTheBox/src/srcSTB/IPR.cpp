...
//	Mat_Close(matfp);
	...
	int* tmp = new int[sizeofpos3D * Npixw * Npixh];
	for (int i = 0; i < sizeofpos3D; i++) {
		for (int j = 0; j < Npixw; j++ ) {
			for (int k = 0; k < Npixh; k++) tmp[i * sizeofpos3D + j * Npixw + k] = pix[i][j][k];
		}
	}
	NumDataIO<int> data_io;
...
//	Mat_Close(mat);
	...
	double* points_array = new double [rows * 8];
	data_io.ReadData((double*) points_array);
	for (int id = 0; id < ncams; id++) {
		Frame pos2D;
		if (id != ignoreCam) {
			for (int i = 0; i < rows; i++) {
				Position pos(points_array[i + 2 * id * ncams], points_array[i + 1 + (2 * id) * ncams], 0);
				pos2D.Add(pos);
			}
		}
		iframes.push_back(pos2D);
...
deque<Position> IPR::Array2Position(int num_particle, double* array) {
	...
	for (int i = 0; i < num_particle; i++) {
		Position point(array[i * num_particle +  0], array[i * num_particle + 1], array[i * num_particle + 2], 
			...
			array[i * num_particle + 9], array[i * num_particle + 10], array[i * num_particle + 11]);
		pos.push_back(point);
	}
	return pos;
}
...
void IPR::SaveParticleIntensity(deque<double> intensity, string file_path) {
	...
	data_io.SetFilePath(file_path);
	double* intensity_array = new double[num]; // Convert deque into double
	for (int i = 0; i <  num; i++) {
...
deque<double> IPR::ReadParticleIntensity(string file_path) {
	...
	double* intensity_array = new double[num];
