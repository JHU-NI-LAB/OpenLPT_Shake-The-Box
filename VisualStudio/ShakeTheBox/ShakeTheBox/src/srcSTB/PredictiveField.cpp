...
// TODO: to check whether it works
	...
	size_t sizeofpos3D = totalGridPoints;
	double* point_array = new double [sizeofpos3D * 3];
	for (int i = 0; i < sizeofpos3D; i++) {
		point_array[i * sizeofpos3D + 0] = pos[0][i];
		point_array[i * sizeofpos3D + 1] = pos[1][i];
		point_array[i * sizeofpos3D + 2] = pos[2][i];
	}
...
//	Mat_Close(matfp);
	size_t sizeofpos3D = totalGridPoints;
	double* point_array = new double[sizeofpos3D * 6];
	for (int i = 0; i < sizeofpos3D; i++) {
		point_array[i * sizeofpos3D + 0] = pos[i][0];
		point_array[i * sizeofpos3D + 1] = pos[i][1];
		point_array[i * sizeofpos3D + 2] = pos[i][2];
		point_array[i * sizeofpos3D + 3] = pos[i][3];
		point_array[i * sizeofpos3D + 4] = pos[i][4];
		point_array[i * sizeofpos3D + 5] = pos[i][5];
	}
